...
// CSM multiview: one draw writes all cascade layers via gl_ViewIndex.
...
layout(push_constant) uniform ShadowPushConstant {
    mat4 lightVP;       // unused for CSM multiview (kept for layout match)
    vec4 lightPosFar;
...
void main() {
    ...
    gl_Position = shadows.cascadeViewProj[gl_ViewIndex] * worldPos;
}
